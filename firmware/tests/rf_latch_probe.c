...
static void rf_l5937_rx_latch_candidate(void)
{
    latch_write(0xEE, 0x7E);
    latch_write(0x5E, 0x0E);
}

static void rf_l5948_rx_alt_latch_candidate(void)
{
    latch_write(0xAE, 0x8E);
}

static void rf_l5964_tx_latch_candidate(void)
{
    latch_write(0xEE, 0x7E);
    latch_write(0x5E, 0xFE);
}

static void rf_l4de5_tx_timer_latch_a(void)
{
    latch_write(0x8E, 0x1E);
}

static void rf_l4de5_tx_timer_latch_b(void)
{
    latch_write(0xCE, 0xEE);
}

static void rf_l5977_stop_candidate(void)
{
    XBYTE(0x224C) = 0xFF;
    XBYTE(0x224D) = 0xFF;
}
...
{
    panel_show_text8("1RX 2TX ");
}

static void probe_rx_candidate(void)
{
    rf_l5937_rx_latch_candidate();
    rf_l5948_rx_alt_latch_candidate();
    uart_send_panel(PANEL_IO_ANT_OFF);
    uart_send_panel(PANEL_IO_ANT_GREEN_ON);
    panel_show_text8("RX  69  ");
}

static void probe_tx_candidate(void)
{
    rf_l5964_tx_latch_candidate();
    rf_l4de5_tx_timer_latch_a();
    short_delay();
    rf_l4de5_tx_timer_latch_b();
    uart_send_panel(PANEL_IO_ANT_ORANGE_ON_B);
    panel_show_text8("TX  68  ");
}

static void probe_stop_candidate(void)
{
    rf_l5977_stop_candidate();
    uart_send_panel(PANEL_IO_ANT_OFF);
    panel_show_text8("RF OFF  ");
}
